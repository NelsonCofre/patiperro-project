...
@Component
public class PaseadorCorreoRegistroClient {
    ...
    public PaseadorCorreoRegistroClient(
            @Value("${patiperro.tutores.integracion.paseadores.base-url:http://localhost:8082}") String baseUrl,
            @Value("${patiperro.tutores.integracion.paseadores.interno.secret:}") String internoSecret) {
        ...
        try {
            CorreoExisteResponse body = restClient.get()
                    .uri(uriBuilder -> uriBuilder
                            .path("/api/paseadores/interno/correo/existe")
